        ...
        switch (state) {
            ...
            case 1://Got next move, do something based on direction
                ...
                if (dx < -movethresh && dirx) {
                    console.log('right');
                    document.getElementsByTagName('h1')[0].innerText = 'Right';
                }
                else if (dx > movethresh && dirx) {
                    console.log('left');
                    document.getElementsByTagName('h1')[0].innerText = 'Left';
                }
                if (dy > movethresh && !dirx) {
                    if (davg > overthresh) {
                        console.log('over up');
                        document.getElementsByTagName('h1')[0].innerText = 'Over Up';
                    }
                    else {
                        console.log('up');
                        document.getElementsByTagName('h1')[0].innerText = 'Up';
                    }
                }
                else if (dy < -movethresh && !dirx) {
                    if (davg > overthresh) {
                        console.log('over down');
                        document.getElementsByTagName('h1')[0].innerText = 'Over Down';
                    }
                    else {
                        console.log('down');
                        document.getElementsByTagName('h1')[0].innerText = 'Down';                        
                    }
                }
                state = 2
                break
